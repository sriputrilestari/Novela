                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <div class="small text-muted mt-1">Reader ini mengajukan diri menjadi author. Tinjau dan ambil
                                keputusan.</div>
                        </div>
                    </div>
            ...
            <div class="col-xl-4 col-lg-5 mb-4">
                ...
                <div class="card shadow mb-4">
                    <div class="card-body text-center pt-4 pb-3">
                        <div class="mb-3">
                            @if ($reader->photo)
                                <img src="{{ asset('storage/' . $reader->photo) }}" class="rounded-circle"
                    ...
                    <div class="card-body p-0">
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                                ...
                                <span
                                    class="small font-weight-bold">{{ $reader->created_at?->format('d M Y') ?? '-' }}</span>
                            </li>
                            <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                                <span class="text-muted small"><i class="fas fa-sync-alt fa-fw mr-2"></i>Terakhir
                                    Update</span>
                                <span
                                    class="small font-weight-bold">{{ $reader->updated_at?->diffForHumans() ?? '-' }}</span>
                            </li>
                            @if ($reader->author_request_date)
                                <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                                    <span class="text-muted small"><i class="fas fa-paper-plane fa-fw mr-2"></i>Tgl
                                        Pengajuan</span>
                                    <span
                                        class="small font-weight-bold">{{ \Carbon\Carbon::parse($reader->author_request_date)->format('d M Y') }}</span>
                                </li>
                            @endif
                            <li class="list-group-item d-flex justify-content-between align-items-center px-4 py-3">
                ...
                @if ($reader->author_request_note)
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body p-0">
                            @php
                                $note = $reader->author_request_note;
                                $parts = [];
                                foreach (explode("\n\n", $note) as $line) {
                                    if (str_starts_with($line, 'Nama pena: ')) {
                                        $parts['nama'] = substr($line, strlen('Nama pena: '));
                                    } elseif (str_starts_with($line, 'Sinopsis: ')) {
                                        $parts['sinopsis'] = substr($line, strlen('Sinopsis: '));
                                    } elseif (str_starts_with($line, 'Pengalaman: ')) {
                                        $parts['pengalaman'] = substr($line, strlen('Pengalaman: '));
                                    } elseif (str_starts_with($line, 'Motivasi: ')) {
                                        $parts['motivasi'] = substr($line, strlen('Motivasi: '));
                                    } elseif (str_starts_with($line, 'Genre favorit: ')) {
                                        $parts['genres'] = substr($line, strlen('Genre favorit: '));
                                    }
                                }
                                $levels = [
                                    'pemula' => 'Pemula',
                            ...
                            <ul class="list-group list-group-flush">
                                @if (!empty($parts['nama']))
                                    <li class="list-group-item px-4 py-3">
                                        <div class="text-xs font-weight-bold text-uppercase text-gray-500 mb-1">Nama Pena
                                        </div>
                                        <div class="text-gray-800">{{ $parts['nama'] }}</div>
                                    </li>
                                @endif

                                @if (!empty($parts['sinopsis']))
                                    <li class="list-group-item px-4 py-3">
                                        <div class="text-xs font-weight-bold text-uppercase text-gray-500 mb-1">Sinopsis
                                            Novel Pertama</div>
                                        <div class="text-gray-700 small" style="line-height:1.7">{{ $parts['sinopsis'] }}
                                        </div>
                                    </li>
                                @endif

                                @if (!empty($parts['pengalaman']))
                                    <li class="list-group-item px-4 py-3">
                                        <div class="text-xs font-weight-bold text-uppercase text-gray-500 mb-1">Pengalaman
                                            Menulis</div>
                                        <div class="text-gray-800">
                                            {{ $levels[$parts['pengalaman']] ?? $parts['pengalaman'] }}</div>
                                    </li>
                                @endif

                                @if (!empty($parts['motivasi']))
                                    <li class="list-group-item px-4 py-3">
                                        <div class="text-xs font-weight-bold text-uppercase text-gray-500 mb-1">Motivasi
                                        </div>
                                        <div class="text-gray-700 small" style="line-height:1.7">{{ $parts['motivasi'] }}
                                        </div>
                                    </li>
                                @endif

                                @if (!empty($parts['genres']))
                                    <li class="list-group-item px-4 py-3">
                                        <div class="text-xs font-weight-bold text-uppercase text-gray-500 mb-1">Genre
                                            Favorit</div>
                                        <div>
                                            @foreach (explode(', ', $parts['genres']) as $genreId)
                                                @php $g = \App\Models\Genre::find(trim($genreId)); @endphp
                                                @if ($g)
                                                    <span
                                                        class="badge badge-pill badge-light border mr-1">{{ $g->nama_genre }}</span>
                                                @endif
                                            @endforeach
                                        </div>
                                    </li>
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.reader.block', $reader->id) }}" method="POST" class="mb-2"
                            ...
                            @csrf
                            <button type="submit"
                                class="btn btn-block btn-sm {{ $reader->is_active ? 'btn-danger' : 'btn-success' }}">
                        ...
                        <div class="card border-left-primary shadow h-100 py-2">
                            <div class="card-body py-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">
                                    {{ $reader->reading_histories_count ?? 0 }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4 mb-3 mb-sm-0">
                        <div class="card border-left-success shadow h-100 py-2">
                            <div class="card-body py-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $reader->comments_count ?? 0 }}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-4">
                        <div class="card border-left-info shadow h-100 py-2">
                            <div class="card-body py-2">
                                ...
                                <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $reader->ratings_count ?? 0 }}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body p-0">
                        @forelse($readingHistories ?? [] as $history)
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover mb-0">
                                        <thead style="background-color: #f8f9fc;">
                                            <tr>
                                                <th
                                                    class="border-0 px-4 py-3 text-xs font-weight-bold text-uppercase text-gray-600">
                                                    Novel</th>
                                                <th
                            ...
                            @endif
                            <tr>
                                <td class="px-4 align-middle font-weight-bold text-gray-800">
                                    {{ $history->chapter?->novel?->judul ?? '-' }}</td>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body p-0">
                    @forelse($comments ?? [] as $comment)
                        ...
                        <tr>
                            <td class="px-4 align-middle font-weight-bold text-gray-800">
                                {{ $comment->chapter?->novel?->judul ?? '-' }}</td>
                            <td class="align-middle text-muted small">
                                {{ \Illuminate\Support\Str::limit($comment->komentar, 60) }}</td>
                            <td class="align-middle text-muted small">
