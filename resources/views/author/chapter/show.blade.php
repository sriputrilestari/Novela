...
<style>
:root{--blue:#3d5af1;--blue-lt:#eef0fe;--blue-md:#dde2fc;--green:#00c9a7;--green-lt:#e0faf5;--amber:#f1a83d;--amber-lt:#fef6e6;--red:#f1523d;--red-lt:#fef0ee;--ink:#18192a;--ink-2:#5a5f7a;--ink-3:#9698ae;--line:#e8eaf3;}
...
/* buttons */
.btn-edit{display:inline-flex;align-items:center;gap:7px;background:var(--blue);color:#fff;border:none;border-radius:10px;padding:10px 20px;font-family:inherit;font-size:13px;font-weight:700;text-decoration:none;transition:.2s;white-space:nowrap;}
.btn-edit:hover{background:#2d48e0;transform:translateY(-1px);color:#fff;}
.btn-back{display:inline-flex;align-items:center;gap:7px;background:#f4f6fc;color:var(--ink-2);border:1px solid var(--line);border-radius:10px;padding:10px 18px;font-family:inherit;font-size:13px;font-weight:700;text-decoration:none;transition:.2s;}
.btn-back:hover{background:var(--line);color:var(--ink);}

/* meta bar */
.meta-bar{display:flex;align-items:center;gap:12px;flex-wrap:wrap;background:#fff;border:1px solid var(--line);border-radius:16px;padding:16px 24px;margin-bottom:20px;box-shadow:0 2px 12px rgba(24,25,42,.06);animation:up .4s .05s ease both;}
.meta-item{display:flex;align-items:center;gap:8px;}
.meta-ico{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:14px;}
.meta-lbl{font-size:11.5px;font-weight:600;color:var(--ink-3);}
.meta-val{font-size:13px;font-weight:700;color:var(--ink);}
.meta-sep{width:1px;height:28px;background:var(--line);}
.pill{border-radius:99px;padding:4px 12px;font-size:11.5px;font-weight:700;}
.pill-pub{background:var(--green-lt);color:#00a08a;border:1px solid rgba(0,201,167,.18);}
.pill-dft{background:#f4f6fc;color:var(--ink-3);border:1px solid var(--line);}

/* content card */
.ccard{background:#fff;border:1px solid var(--line);border-radius:16px;box-shadow:0 2px 12px rgba(24,25,42,.06);overflow:hidden;animation:up .4s .1s ease both;}
.ccard-head{padding:16px 24px;border-bottom:1px solid var(--line);background:#f8f9fe;display:flex;align-items:center;justify-content:space-between;}
.ccard-title{font-size:14px;font-weight:700;color:var(--ink);}
.word-count{font-size:12px;font-weight:600;color:var(--ink-3);}
.ccard-body{padding:28px 32px;}

/* chapter content */
.chapter-text{font-size:15px;line-height:1.95;color:#2a2b3a;white-space:pre-wrap;word-break:break-word;}

/* timestamps */
.ts-bar{display:flex;align-items:center;gap:20px;flex-wrap:wrap;margin-top:24px;padding-top:20px;border-top:1px solid var(--line);}
.ts-item{font-size:12px;color:var(--ink-3);display:flex;align-items:center;gap:5px;}
</style>
...
<div class="ch">
    ...
    <div class="pg-head">
        ...
        <div class="pg-actions">
            <a href="{{ route('author.chapter.edit', [$novel->id, $chapter->id]) }}" class="btn-edit">✏️ Edit Chapter</a>
            <a href="{{ route('author.chapter.index', $novel->id) }}" class="btn-back">← Kembali</a>
        </div>
    </div>
    ...
    <div class="ccard">
        <div class="ccard-head">
            ...
            <span class="word-count" id="wordCount2"></span>
        </div>
        <div class="ccard-body">

            <div class="chapter-text" id="chapterText">{{ $chapter->isi }}</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!text) return;
    const words = text.textContent.trim().split(/\s+/).filter(w => w).length;
    const label = words.toLocaleString('id-ID') + ' kata';
    const el1   = document.getElementById('wordCount');
    const el2   = document.getElementById('wordCount2');
